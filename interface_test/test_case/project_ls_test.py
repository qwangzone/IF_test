...
class ProjectList(unittest.TestCase):
    ...
    @parameterized.expand([('web_pagesize10', 'WEB', '', '1', '4', '', '', '','4'),
                           ('web_pagesize10', 'WEB', '', '1', '8', '', '', '', '8'),
                           ])
    def test_projectls_pagesize(self, name, source, projectCategory, page, pageSize,
                              projectStatus, searchTime, projectNewType,assert_text):
        stampToken = str(get_stampToken())
        test_data1 = {'stampToken': stampToken, 'source': source,
                      'page': page, 'pageSize': pageSize, 'searchTime': searchTime,
                      'projectNewType': projectNewType, 'projectCategory': projectCategory,
        ...
        print(result)
        self.assertEqual(result['code'], 1)
        self.assertEqual(len(result['item']['list']), int(assert_text))
